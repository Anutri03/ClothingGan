...
def setup_model():
    ...
    # Initialize config with proper output class
    config = Config(
        model=model_name,
        layer=layer_name,
...
def setup_ui():
    global root, toolbar, ui_state, canvas
...
def resample_latent(seed=None, only_style=False):
    ...
    if B > 1:
        print(f'Seeds: {state.seed} -> {state.seed + B - 1}')
    else:
...
def on_draw():
    ...
    # Display image using matplotlib instead of OpenGL
    plt.figure(1)
    plt.clf()
...
def main():
    try:
        ...
        setup_model()
        setup_ui()
        resample_latent()  # Initialize latent vector after UI is set up
